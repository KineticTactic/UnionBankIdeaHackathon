...
def compute_v2_score(c):
    ...
    noise = lambda s: ((seed * s * 7) % 11 - 5.5) / 100  # deterministic ±5.5%
    ...
    # Conformal prediction CI (±disagreement/2 roughly)
...
def make_action_plan(c, score):
    ...
    rationale = (
        ...
        f"Conformal CI [{score['conformal_lower']:.2f}–{score['conformal_upper']:.2f}]. "
...
Customer: {c['full_name']}, {seg} segment, {tenure} year customer of Union Bank
...
Customer: {name}, {seg}, {tenure} year customer of Union Bank
Offer: {offer_desc} worth {offer_val}
Tone: {tone_str}
...
Only return the JSON."""
    ...
    if chan == "email":
        result["subject_line"] = parsed.get("subject_line", f"An exclusive offer from Union Bank, {name}")
        ...
        result["ab_variant"]   = {
            "subject_line": f"{name}, your Union Bank loyalty reward awaits",
            "preview_text": f"Exclusive for {seg} members like you",
        }
    elif chan == "sms":
        result["body"] = raw if raw and len(raw) <= 160 else f"Hi {name}! Union Bank exclusive: {offer_desc} worth {offer_val}. Reply YES for details or STOP to opt out."
    elif chan == "app":
        result["title"] = parsed.get("title", f"Exclusive for you, {name}")
        result["body"]  = parsed.get("body", f"{offer_desc} — activated for your account")
        result["cta"]   = parsed.get("cta", "Claim Now")
    else:
        result["opening"]       = parsed.get("opening", f"Good morning, this is your Relationship Manager from Union Bank calling regarding your account.")
        result["talking_points"] = parsed.get("talking_points", [
            f"Acknowledge {tenure}-year relationship and express genuine appreciation",
            f"Present {offer_desc} worth {offer_val} — personalised based on their profile",
