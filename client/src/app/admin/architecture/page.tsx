...
const LAYERS = [
  { id: 'l1',      x: 20,  y: 140, label: 'L1 Bank API',    sub: 'Data Ingestion',          color: 'var(--gray-500)' },
  { id: 'argus',   x: 210, y: 140, label: 'L2 ARGUS',       sub: '9 Signal Detectors',      color: 'var(--crimson)' },
  { id: 'chronos', x: 400, y: 140, label: 'L3 CHRONOS',     sub: 'ML Ensemble + GraphSAGE', color: 'var(--crimson)' },
  { id: 'compass', x: 590, y: 140, label: 'L4 COMPASS',     sub: 'LangGraph Orchestration', color: 'var(--crimson)' },
  { id: 'herald',  x: 780, y: 140, label: 'L5 HERALD',      sub: 'Content Generation',      color: 'var(--crimson)' },
  { id: 'verdict', x: 590, y: 300, label: 'L6 VERDICT',     sub: 'DR Uplift Measurement',   color: 'var(--teal-dark)' },
  { id: 'oracle',  x: 400, y: 300, label: 'L7 ORACLE',      sub: 'Continuous Learning',     color: 'var(--sage-brand)' },
];
const EDGES = [
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
        <svg viewBox="0 0 980 420" className="w-full" style={{ height: 400 }}>
          <defs>
            <marker id="a1" markerWidth="8" markerHeight="8" refX="6" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 Z" fill="var(--gray-400)" /></marker>
            <marker id="a2" markerWidth="8" markerHeight="8" refX="6" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 Z" fill="var(--copper)" /></marker>
          </defs>
          ...
          {EDGES.map((e, i) => {
            ...
            return (
              <g key={i}>
                <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="var(--gray-300)" strokeWidth={2} markerEnd="url(#a1)" />
                <text x={(x1+x2)/2} y={e.ly} textAnchor="middle" fontSize="11" fill="var(--gray-400)">{e.label}</text>
                <circle r="4" fill="var(--crimson)">
            ...
            return (
              <g key={i}>
                <path d={`M${x1},${y1} L${x1},${my} L${x2},${my} L${x2},${y2}`}
                  fill="none" stroke="var(--copper)" strokeWidth={1.8} strokeDasharray="6 4" markerEnd="url(#a2)" />
                <text x={(x1+x2)/2} y={my - 6} textAnchor="middle" fontSize="10" fill="var(--copper)">{e.label}</text>
              </g>
            );
          })}
            ...
            return (
              <g key={l.id} onClick={() => setSelected(s => s === l.id ? null : l.id)} className="cursor-pointer" style={{ userSelect: 'none' }}>
                <rect x={l.x} y={l.y} width={NW} height={NH} rx={10}
                  fill={isSel ? l.color : 'var(--background)'} stroke={isSel ? l.color : 'var(--border-color)'} strokeWidth={isSel ? 2.5 : 1.5} />
                <text x={l.x+NW/2} y={l.y+24} textAnchor="middle" fontSize="12" fontWeight="700" fill={isSel ? '#fff' : 'var(--gray-700)'}>{l.label}</text>
                <text x={l.x+NW/2} y={l.y+42} textAnchor="middle" fontSize="10" fill={isSel ? 'rgba(255,255,255,0.7)' : 'var(--gray-400)'}>{l.sub}</text>
                {h && <circle cx={l.x+NW-10} cy={l.y+10} r={5} fill={h.status==='live'?'var(--sage-brand)':'var(--crimson)'} />}
              </g>
            );
          })}
        </svg>
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h3 className="font-bold text-slate-900 text-base">{selL.label}
                {selH && <span className={`ml-2 text-[10px] font-bold px-2 py-0.5 rounded ${selH.status==='live'?'bg-sage-soft text-sage-brand':'bg-crimson-soft text-crimson'}`}>{selH.status?.toUpperCase()} · {selH.latency_ms}ms</span>}
              </h3>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex items-center gap-4 flex-wrap">
        ...
        {['pcop.alarms.v1','pcop.action_plans.v1','pcop.dispatched.v1','pcop.measurements.v1'].map(t => (
          <span key={t} className="text-[11px] font-mono bg-slate-100 text-[var(--crimson)] px-2.5 py-1 rounded-lg border border-slate-200">{t}</span>
        ))}
      </div>
    </div>
  );
