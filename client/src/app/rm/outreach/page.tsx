...
const STATUS_COLORS: Record<string, string> = {
  sent:'bg-sage-soft text-sage-brand', delivered:'bg-teal-soft text-teal-dark',
  opened:'bg-teal-soft text-teal-dark', clicked:'bg-teal-soft text-teal-dark',
  pending:'bg-copper-soft text-copper-dark', approved:'bg-sage-soft text-sage-brand',
  rejected:'bg-crimson-soft text-crimson',
};
      ...
      <div className="flex gap-1 mb-6 border-b border-slate-100">
        {(['sent','pending','blocked'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2.5 text-[13px] font-medium capitalize border-b-2 -mb-px transition-colors ${tab===t ? 'border-[var(--crimson)] text-[var(--crimson)]' : 'border-transparent text-slate-400 hover:text-slate-600'}`}>
            {t}
            {t === 'pending' && approvals.filter(a=>a.status==='pending').length > 0 && (
              <span className="ml-1.5 px-1.5 py-0.5 rounded-full bg-copper-soft text-copper-dark text-[10px] font-bold">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          {outreach.length === 0 ? (
            ...
            <div key={o.id} className="flex items-center gap-3 px-4 py-3 border-b border-slate-50 last:border-0">
              <Link href={`/rm/customers/${o.customer_id}`} className="w-32 text-[12px] font-medium text-[var(--crimson)] hover:underline truncate">{o.customer_id}</Link>
          ...
          {approvals.filter(a=>a.status==='pending').length === 0 ? (
            ...
            <div key={a.id} className="bg-white border border-slate-200 rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[10px] font-semibold px-2 py-0.5 rounded bg-copper-soft text-copper-dark">Pending</span>
              </div>
              ...
              <div className="flex gap-2">
                <button onClick={() => approve(a.id)} disabled={acting===a.id}
                  className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-sage-brand text-white text-[11px] font-semibold hover:bg-sage-brand disabled:opacity-50 transition-colors">
                ...
                <button onClick={() => reject(a.id)} disabled={acting===a.id}
                  className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-soft text-crimson text-[11px] font-semibold hover:bg-crimson-soft disabled:opacity-50 transition-colors">
