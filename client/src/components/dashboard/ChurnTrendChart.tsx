...
function CustomTooltip({ active, payload, label }: any) {
    ...
    return (
        <div className="bg-white border border-slate-200 rounded-lg shadow-sm px-3 py-2">
            ...
            <p className="text-sm font-bold text-teal-dark">{(payload[0]?.value * 100).toFixed(1)}% avg churn risk</p>
        </div>
    );
}
    ...
    return (
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm h-full">
            <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-teal-soft text-teal-dark border border-blue-100 uppercase tracking-wider">12 weeks</span>
            </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={data} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                        ...
                        <XAxis
                            ...
                            tick={{ fontSize: 11, fill: 'var(--gray-400)' }} dy={8}
                        />
                        <YAxis
                            domain={[0.2, 0.9]}
                            axisLine={false} tickLine={false}
                        ...
                        <Area
                            ...
                            fill="url(#churnGrad)"
                            dot={false}
                            activeDot={{ r: 4, fill: "var(--teal)", stroke: "#fff", strokeWidth: 2 }}
                        />
                    </AreaChart>
                </ResponsiveContainer>
            </div>
