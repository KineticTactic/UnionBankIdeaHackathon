...
const AZURE_ENDPOINT = 'https://kensara.services.ai.azure.com/models/chat/completions?api-version=2024-05-01-preview';
const AZURE_KEY      = 'AcBR2FelM3JboPVb4kchmdFNOdXjkyWcgg5oHfB1pFpTpLbu1Iz4JQQJ99CEACYeBjFXJ3w3AAAAACOGLc3P';
const AZURE_MODEL    = 'DeepSeek-V4-Pro-4';

function loadV2Score(customerId) {
    try {
        const all = JSON.parse(fs.readFileSync(path.join(__dirname, '../../chronos/data/scores_v2.json'), 'utf8'));
...
function loadActionPlan(customerId) {
    try {
        const all = JSON.parse(fs.readFileSync(path.join(__dirname, '../../chronos/data/action_plans.json'), 'utf8'));
...
function loadCrmNotes(customerId) {
    try {
        const all = JSON.parse(fs.readFileSync(path.join(__dirname, '../../bank/data/crm_notes.json'), 'utf8'));
...
function buildOutreachPrompt(channel, customerData) {
    const { customer, engagement, active_signals, life_events, v2Score, accounts, crmNotes, actionPlan } = customerData;
    ...
    const acctSection = (accounts || []).length
        ? accounts.map(a => `  ${a.account_type} (${a.product_code}) — ₹${Number(a.balance || 0).toLocaleString('en-IN')} — status: ${a.status}`).join('\n')
        : '  N/A';
    ...
    const survival7d  = v2Score ? `${(v2Score.survival_7d  * 100).toFixed(1)}%` : 'N/A';
    const survival30d = v2Score ? `${(v2Score.survival_30d * 100).toFixed(1)}%` : 'N/A';
    const lastLogin   = engagement?.days_since_last_login != null ? `${engagement.days_since_last_login} days ago` : 'unknown';
...
router.post('/generate', verifyToken, async (req, res) => {
    ...
    if (!customer_id || !channel) {
        return res.status(400).json({ status: 'error', message: 'customer_id and channel are required' });
    }
    ...
    const active_signals = dataStore.SIGNALS.filter(s => s.customer_id === customer_id && s.detected);
    const life_events    = dataStore.LIFE_EVENTS.filter(e => e.customer_id === customer_id);
    const v2Score        = loadV2Score(customer_id);
    const actionPlan     = loadActionPlan(customer_id);
    const crmNotes       = loadCrmNotes(customer_id);

    const prompt = buildOutreachPrompt(channel, { customer, accounts, engagement, crm_summary, active_signals, life_events, v2Score, actionPlan, crmNotes });
        ...
        const azureReq = https.request(options, (azureRes) => {
            ...
            azureRes.on('data', (chunk) => {
                ...
                for (const line of lines) {
                    ...
                    if (raw === '[DONE]') { sendEvent({ type: 'done' }); res.end(); return; }
                    try {
            ...
            azureRes.on('end', () => { sendEvent({ type: 'done' }); res.end(); });
            azureRes.on('error', (e) => { sendEvent({ type: 'error', message: e.message }); res.end(); });
        });
